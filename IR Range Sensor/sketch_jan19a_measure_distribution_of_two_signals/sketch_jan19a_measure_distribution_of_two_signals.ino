// Tried using analogNoiseReducedRead here, but it causes the arduino to reset.
...
void loop() {
  ...
  if (doMeasure) {
    ...
    for (int ndx = 0; ndx < analogSamples; ++ndx) {
      lowPinSum += analogNoiseReducedRead(LOW_PIN);
      highPinSum += analogNoiseReducedRead(HIGH_PIN);
    }
