...
bool integerFade(uint8_t reduce) {
 ...
 public:
  Triangle(uint16_t a, uint16_t b, uint16_t c, uint16_t d, uint8_t lo, uint8_t hi)
      : a_(a), b_(b), c_(c), d_(d - d % NPIXELS), step_(d_ / NPIXELS),
        lo_(lo), hi_(hi), hi_lo_(hi - lo) {}

  uint8_t intensity(unsigned long time, uint16_t pixel, bool forwards) {
    if (!forwards) {
      pixel = NPIXELS - pixel;
    }
    uint16_t pos = (uint32_t(pixel) * step_ + time) % d_;
...
void colorChase() {
  unsigned long time = millis();
  const unsigned long end = time + 40L * 1000L;
  unsigned long last_time = 0;

  bool redForward = random(0, 2) == 0;
  bool greenForward = random(0, 2) == 0;
  bool blueForward = random(0, 2) == 0;
  ...
  while (true) {
    ...
    if (time != last_time) {
      ...
      for (uint16_t pixel = 0; pixel < NPIXELS; ++pixel) {
        int r = red_tri.intensity(time, pixel, redForward);
        int g = green_tri.intensity(time, pixel, greenForward);
        int b = blue_tri.intensity(time, pixel, blueForward);
    ...
    strip.show();
    if (10 == random(0, 3000)) {
      redForward = random(0, 2) == 0;
      greenForward = random(0, 2) == 0;
      blueForward = random(0, 2) == 0;
    }
  }
}
