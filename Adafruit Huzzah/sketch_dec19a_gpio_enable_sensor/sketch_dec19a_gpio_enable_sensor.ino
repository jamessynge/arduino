 ...
 * 8) Enter deepSleep.
 *
 * STATUS: I restarted my laptop and then this program was no longer able
 * to connect to the server, even though the address didn't change. Perhaps
 * the problem is that the server is listening on the wrong interface (the
 * laptop has both wired and wireless connections).
 * 
 * STATUS: It works to power the sensor via a GPIO pin.
 * 
 * Change Log:
 ...
 * 
 * TODO Report to io.adafruit.com instead of to local server, so not dependent upon
...
// // Need to wait a bit longer for some reason to reach 5 minutes.
// const int report_interval_secs = 5 * 60 + 14; 
const int report_interval_secs = 60 * 60 + 14;
